...
class BMIApp(tk.Tk):
    def __init__(self):
        ...
        self.button_plot_history = tk.Button(self, text="Plot History", command=self.plot_history)
        self.button_plot_history.pack()
    ...
    def plot_history(self):
        ...
        if data:
            ...
            plt.plot(dates, heights, label='Height')
            plt.xlabel('Date')
            plt.ylabel('Value')
            ...
            # Embed plot into tkinter application
            fig_canvas = FigureCanvasTkAgg(plt.gcf(), master=self)
            fig_canvas.draw()
